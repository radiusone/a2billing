...
if (!empty($type) && !empty($view_type)) {
    ...
    switch ($type) {
        case "refills_count":
            $query = "SELECT UNIX_TIMESTAMP(DATE_FORMAT(date, '$dt_fmt')) * 1000 AS period, COUNT(*) FROM cc_logrefill WHERE date >= TIMESTAMP('$ck_dt') AND date <= CURRENT_TIMESTAMP GROUP BY period ORDER BY period";
            break;
        case "refills_amount":
            $query = "SELECT UNIX_TIMESTAMP(DATE_FORMAT(date, '$dt_fmt')) * 1000 AS period, SUM(credit) FROM cc_logrefill WHERE date >= TIMESTAMP('$ck_dt') AND date <= CURRENT_TIMESTAMP GROUP BY period ORDER BY period";
            $format = "money";
            break;
        default:
            die();
