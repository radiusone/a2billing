...
getpost_ifset(["result", "id", "action", "status", "newcomment"]);
/**
 * @var string|null $result
 * @var numeric-string|null $id
 * @var string|null $action
 * @var numeric-string|null $status
 * @var string|null $newcomment
 */
    ...
    <div class="col-6">
        <table class="table table-sm caption-top">
            <caption class="fw-bold fs-5">
                <?= _("Ticket Info") ?>
                <?php if ($ticket->getViewed(Ticket::CUSTOMER)):?>
                <span class="badge text-bg-danger"><?= _("NEW") ?></span>
                <?php endif ?>
                <?php $ticket->markViewed(Ticket::CUSTOMER) ?>
            </caption>
            <tbody>
                <tr>
    ...
    <form action="?id=<?= $ticket->getId() ?>" method="post" class="col-6">
        <div class="mb-3">
            ...
            <select name="status" id="status" class="form-select form-select-sm">
                <?php foreach (Ticket::getPossibleStatus($ticket->getStatus()) as $state): ?>
                <option value="<?= $state["id"] ?>"><?= $state["name"] ?></option>
                <?php endforeach ?>
            </select>
        </div>
        <div class="mb-3">
            <label for="newcomment" class="form-label form-label-sm"><?= _("Comment") ?></label>
            <textarea name="newcomment" id="newcomment" class="form-control form-control-sm" rows="5"></textarea>
        </div>
        <div class="my-4">
        ...
        <table class="table table-sm caption-top">
            ...
            <?php foreach ($ticket->loadComments() as $comment): ?>
                <tr>
                    ...
                    <?php endif ?>
                    <?php $comment->markViewed(Comment::CUSTOMER) ?>
                    </td>
                </tr>
            <?php endforeach ?>
            </tbody>
