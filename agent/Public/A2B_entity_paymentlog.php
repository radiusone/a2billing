...
if ($form_action != "list" && isset ($id)) {
    if (!empty ($id) && $id > 0) {
        $table_agent_security = new Table(
            "cc_epayment_log",
            ["cc_card_group.id_agent"],
            ["cc_card" => ["cc_card.id", "cc_epayment_log.cardid"], "cc_card_group" => ["cc_card.id_group", "cc_card_group.id"]]
        );
        $clause_agent_security = ["cc_epayment_log.id" => $id];
